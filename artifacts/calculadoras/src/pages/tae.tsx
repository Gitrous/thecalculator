...
const T = {
  es: {
    ...
    intro2: "Esta calculadora convierte en ambas direcciones entre TIN y TAE usando la fórmula estándar: TAE = (1 + TIN/n)^n − 1, donde n es la frecuencia de capitalización anual. También genera una tabla comparativa para visualizar cómo varía la TAE según la frecuencia (diaria, semanal, mensual, trimestral, semestral o anual). Ten en cuenta que esta conversión refleja únicamente el efecto de la capitalización: no añade las comisiones, que en la TAE legal de un préstamo se suman aparte.",
    disclaimer: "Los resultados de la conversión TIN↔TAE son matemáticamente exactos, pero solo reflejan el efecto de la capitalización. La TAE legal de un préstamo suele ser mayor porque incorpora además las comisiones y gastos obligatorios, que esta herramienta no incluye.",
  ...
  en: {
    ...
    intro2: "This calculator converts in both directions between the nominal rate and APR using the standard formula: APR = (1 + nominal/n)^n − 1, where n is the annual compounding frequency. It also generates a comparison table to visualise how the APR varies by frequency (daily, weekly, monthly, quarterly, half-yearly or annual). Note that this conversion reflects only the effect of compounding: it does not add fees, which in a loan's legal APR are included separately.",
    disclaimer: "The nominal-rate↔APR conversion results are mathematically exact, but they only reflect the effect of compounding. A loan's legal APR is usually higher because it also incorporates mandatory fees and charges, which this tool does not include.",
    cardTitle: "Converter",
