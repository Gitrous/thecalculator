...
const CATEGORY_CONTENT: Record<string, CategoryContent> = {
  finanzas: {
    intro: {
      es: "Las calculadoras financieras de thecalculator.tech te ayudan a planificar tus finanzas personales con datos actualizados a 2026. Desde calcular la cuota mensual de una hipoteca o el coste real de un préstamo hasta estimar tu salario neto tras IRPF y Seguridad Social, cada herramienta incluye explicaciones detalladas para que entiendas el resultado, no solo el número. Las herramientas utilizan fórmulas financieras estándar y, cuando el cálculo depende de la normativa española, parámetros procedentes de fuentes oficiales (AEAT, Seguridad Social) que revisamos periódicamente.",
      en: "Our financial calculators help you plan your personal finances with data updated for 2026. From working out your monthly mortgage payment or the true cost of a loan to estimating your net take-home pay after income tax and Social Security, each tool includes detailed explanations so you understand the result, not just the number. The tools use standard financial formulas and, where a calculation depends on Spanish regulation, parameters taken from official sources (AEAT, Social Security) that we review periodically.",
    },
    intro2: {
    ...
    faqs: [
      ...
      },
      {
        ...
        a: { es: "El TIN (Tipo de Interés Nominal) es el interés puro del préstamo, sin comisiones ni gastos adicionales. La TAE (Tasa Anual Equivalente) añade al TIN el efecto de la frecuencia de capitalización y los gastos y comisiones obligatorios que el cliente debe pagar para obtener la financiación, como la comisión de apertura o de estudio. No incluye, en cambio, los seguros vinculados, los gastos de notaría y registro de una hipoteca ni las comisiones que solo se pagan si ocurre algo (amortización anticipada, reclamación de impagados), así que no equivale exactamente al coste total. Aun así, es el mejor indicador homogéneo para comparar préstamos e hipotecas: compáralos por su TAE, no por su TIN, y suma aparte el coste de los productos vinculados que te exijan.", en: "The TIN (nominal interest rate) is the pure interest rate on the loan, excluding fees and additional costs. The APR (TAE) adds to the nominal rate the effect of the compounding frequency plus the mandatory fees and charges the customer must pay to obtain the financing, such as arrangement or study fees. It does not include linked insurance, notary and registry costs on a mortgage, or contingent fees that are only paid if something happens (early repayment, arrears claims), so it is not exactly the total cost. Even so, it is the best homogeneous indicator for comparing loans and mortgages: compare them by APR, not by nominal rate, and add the cost of any required linked products separately." },
      },
      {
        q: { es: "¿Con qué frecuencia se actualizan los datos fiscales?", en: "How often is the tax data updated?" },
        a: { es: "Actualizamos los tramos de IRPF, los tipos de cotización a la Seguridad Social y las tablas de retención al inicio de cada año fiscal, en cuanto la Agencia Tributaria publica los valores oficiales del ejercicio. Ten en cuenta que, mientras no se aprueban nuevos Presupuestos Generales del Estado, pueden prorrogarse los tramos del ejercicio anterior.", en: "We update the income tax brackets, Social Security contribution rates and withholding tables at the start of each tax year, as soon as the Tax Agency publishes the official values for the year. Note that, until a new General State Budget is approved, the previous year's brackets may be carried over." },
      },
      {
  ...
  trabajo: {
    intro: {
      es: "Las calculadoras de trabajo de thecalculator.tech cubren los aspectos laborales más habituales en España: calcular el paro al que tienes derecho tras un despido o ERTE, estimar el finiquito que te corresponde, llevar el registro de horas trabajadas o simular tu cuota como autónomo en el nuevo sistema de cotización por ingresos reales. Las calculadoras que dependen de normativa laboral se revisan tomando como referencia la legislación española vigente y fuentes oficiales (SEPE, Seguridad Social, BOE); los resultados son orientativos y pueden existir excepciones según el convenio, el contrato o tu situación personal.",
      en: "Our work calculators cover the most common employment situations in Spain: calculating the unemployment benefit you are entitled to after a dismissal or furlough, estimating your severance pay, tracking your working hours or simulating your self-employed contribution under the new real-income-based system. Calculators that depend on labour regulation are reviewed against current Spanish legislation and official sources (SEPE, Social Security, BOE); results are indicative and exceptions may apply depending on your collective agreement, contract or personal situation.",
    },
    intro2: {
  ...
  salud: {
    intro: {
      es: "Las calculadoras de salud de thecalculator.tech ofrecen referencias rápidas sobre tu composición corporal y necesidades energéticas diarias, basadas en fórmulas ampliamente utilizadas. El Índice de Masa Corporal (IMC) sigue los criterios de clasificación de la Organización Mundial de la Salud y la calculadora de calorías utiliza la ecuación de Mifflin-St Jeor (1990), una de las más utilizadas para estimar el gasto energético en adultos según la evidencia científica actual.",
      en: "Our health calculators provide quick references on your body composition and daily energy needs, based on widely used formulas. The Body Mass Index (BMI) follows World Health Organisation classification criteria and the calorie calculator uses the Mifflin-St Jeor equation (1990), one of the most widely used equations to estimate energy expenditure in adults according to current scientific evidence.",
    },
    intro2: {
