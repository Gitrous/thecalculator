...
const T = {
  es: {
    ...
    a1: "El TIN (Tipo de Interés Nominal) es el interés puro que el banco aplica al capital prestado, sin incluir comisiones ni gastos adicionales. La TAE (Tasa Anual Equivalente) es el indicador más completo: incorpora el TIN, el efecto de la capitalización y las comisiones y gastos obligatorios para obtener la financiación (apertura, estudio, formalización a cargo del cliente), y se expresa en términos anuales. No incluye los seguros vinculados ni las comisiones que solo se pagan si ocurre algo, como la de amortización anticipada, así que tampoco es exactamente el coste total. A la hora de comparar préstamos entre entidades, siempre debes fijarte en la TAE, no en el TIN, ya que un préstamo con TIN bajo puede ser más caro que otro si tiene muchas comisiones. Esta calculadora usa el TIN para el cálculo de la cuota mensual.",
  ...
  en: {
    ...
    a1: "The nominal interest rate (TIN) is the pure interest the bank charges on the borrowed capital, excluding fees and additional costs. The Annual Percentage Rate (APR or TAE) is the more complete indicator: it incorporates the nominal rate, the effect of compounding and the mandatory fees and charges needed to obtain the financing (arrangement, study and formalisation costs borne by the customer), expressed on an annual basis. It does not include linked insurance or contingent fees that are only paid if something happens, such as early repayment, so it is not exactly the total cost either. When comparing loans from different lenders, always look at the APR rather than the nominal rate, since a loan with a low nominal rate can end up more expensive if it carries many fees. This calculator uses the nominal rate to compute the monthly payment.",
    ...
    interpret: "Look at two figures, not just the payment. The monthly payment tells you whether the loan fits your budget: as a rough reference, some lenders use debt-to-income ratios of around 30–35% of net monthly income, though the appropriate limit depends on your financial situation. The total interest tells you what the borrowed money actually costs you and is the key number for comparing offers. Bear in mind this calculator works with the nominal rate: if the loan carries an arrangement fee, the real cost will be higher and you should look at the APR the lender is legally required to disclose; and if linked insurance is required on top, add it separately, since it falls outside even that APR.",
  },
};
